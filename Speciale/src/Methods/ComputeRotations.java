...
public class ComputeRotations {
	...
	public ComputeRotations(Graph graph){
		this.graph = graph;
	}
	...
	private Port addBestLeg(ArrayList<AuxNode> rotationNodes, ArrayList<AuxEdge> sortedEdges, VesselClass vesselClass, boolean considerUsed){
		...
		for(AuxEdge e : firstNode.getIngoingEdges()){
			...
			if((!e.isUsedInRotation() || !considerUsed) && !newNode.equals(lastNode) && graph.getPort(newNode.getPortId()).getDraft() >= vesselClass.getDraft() && graph.getPort(newNode.getPortId()).isActive() && e.isLegal(vesselClass, graph)){
...
//				double ratio = newDemand / detourTime;
		...
		for(AuxEdge e : lastNode.getOutgoingEdges()){
			...
			if((!e.isUsedInRotation() || !considerUsed) && !newNode.equals(firstNode) && graph.getPort(newNode.getPortId()).getDraft() >= vesselClass.getDraft() && graph.getPort(newNode.getPortId()).isActive() && e.isLegal(vesselClass, graph)){
				double newDemand = e.getAvgLoad();
				double detourTime = getDetourTime(lastNode.getPortId(), firstNode.getPortId(), newNode.getPortId(), vesselClass);
//				double ratio = newDemand / detourTime;
				double ratio = newDemand;
	...
	public static void removePorts(Graph graph){
		ArrayList<Port> servicedPorts = findServicedPorts(graph.getPorts());
	...
	public static Rotation splitRotation(Rotation originalRotation){
		...
		if(newRotationNoVessels + oldRotationNoVessels <= originalNoVessels){
			...
			newRotation = graph.createRotationFromPorts(newRotationIds, vessel, -1);
...
//		fromTo += graph.getData().getBestDistanceElement(fromPortR2, toPortR1, vessel).getDistance();
		...
		if(mergeNoVessels <= previousNoVessels){
			...
			mergeRotation = graph.createRotationFromPorts(mergeRotationPortIds , vessel, -1);
...
//						System.out.println("Entering for-loop at i=" + i + " with unservicedPort.size()=" + unservicedPorts.size());
	...
	public static int calcCostOfPortInsert(VesselClass v, DistanceElement leg, Port insertPort){
		...
		DistanceElement newLeg1 = Data.getBestDistanceElement(leg.getOrigin(), insertPort.getPortData(), v);
		DistanceElement newLeg2 = Data.getBestDistanceElement(insertPort.getPortData(), leg.getDestination(), v);
	...
	public static double costOfCallingPort(Rotation rotation, int portId, Port[] ports){
		double cost = 0;
		Edge preEdge = null;
		Edge postEdge = null;
